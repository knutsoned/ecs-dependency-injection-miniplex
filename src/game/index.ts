import { BitECSManager } from './ecs/BitECS'
...
export function createWalls(config: IGameConfig, world: IWorld) {
  return () => {
...
export function createLauncher(world: IWorld, globalState: IGlobalState) {
  return () => {
    const eid = addEntity(world)
